...
const calloutVariants = cva(
  'relative flex gap-4 rounded-md border p-5 font-sans',
  {
    variants: {
      variant: {
        default: 'bg-surface border-border text-(--color-text-main)',
        subtle: 'bg-surface/30 border-transparent text-(--color-text-main)',
      },
    },
    defaultVariants: {
      variant: 'default',
    ...
    return (
      <div ref={ref} className={cn(calloutVariants({ variant }), className)} {...props}>
        ...
        <div className="flex flex-1 flex-col gap-2">
          {title && (
            <div className="font-serif text-lg font-semibold tracking-tight text-(--color-text-strong)">
              {title}
            </div>
          )}
